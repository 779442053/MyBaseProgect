...
- (void)titlesWidth {
    ...
    for (NSString *title in titlesArray) {
        ...
        if(_spaceMargin && _spaceMargin > 0){
            width += _spaceMargin;
        }
...
- (CGFloat)titleWidth:(NSString *)title {
    ...
    return [title boundingRectWithSize:CGSizeMake(MAXFLOAT, CGRectGetHeight(self.frame)) options:NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading attributes:@{
        NSFontAttributeName :ADOBE_FONT(sys_font) //[UIFont systemFontOfSize:sys_font]
    } context:nil].size.width + _singleW_Add;
}
...
- (void)scrollViewSubviews:(UIScrollView*)scroll {
    ...
    if (titles && _headStyle != SegmentHeadStyleSlide && _showIndex >= 0) {
        UIButton *curBtn = buttonArray[_showIndex];
        if (_fontScale != 1) {
            //curBtn.titleLabel.font = [UIFont systemFontOfSize:_fontSize*_fontScale];
            curBtn.titleLabel.font = ADOBE_FONT(_fontSize*_fontScale);
        }
...
- (void)createBtn:(NSArray *)titlesArr
        addScroll:(UIScrollView*)scroll
           startX:(CGFloat)start_x
      start_index:(NSInteger)start_index {
    BOOL titles = [scroll isEqual:titlesScroll];
    CGFloat width;
    for (NSInteger i = start_index; i < titlesArr.count; i ++) {
        ...
        [button setAttributedTitle:[Utils setNavAttributeStringText:titlesArr[i] AndFontSize:_fontSize AndTextColor:_deSelectColor]
                          forState:UIControlStateNormal];

        button.titleLabel.font = ADOBE_FONT(_fontSize); //[UIFont systemFontOfSize:_fontSize];
...
- (UIView *)lineView {
    ...
    CGRect rect = CGRectMake(5, SCROLL_HEIGHT-_lineHeight, line_w*_lineScale - 10, _lineHeight);
    if(self.lineWidth > 0){
...
- (void)removeSelect {
    ...
    if (_headStyle != SegmentHeadStyleSlide) {
        ...
        for (UIButton *_btn in buttonArray) {
            [_btn setTintColor:[UIColor colorWithRed:de_sel_red
                                               green:de_sel_green
                                                blue:de_sel_blue
                                               alpha:de_sel_alpha]];

            //_btn.titleLabel.font = [UIFont systemFontOfSize:_fontSize];
            _btn.titleLabel.font = ADOBE_FONT(_fontSize);
        }
    }
    else{
...
- (void)setSelectIndex:(NSInteger)index {
    ...
    if([self.delegate respondsToSelector:@selector(didSelectedIndex:andIndex:)]){
        [self.delegate didSelectedIndex:weakSelf andIndex:currentIndex];
    }
    else if (self.selectedIndex2){
       self.selectedIndex2(weakSelf, currentIndex);
    }
...
- (void)changeBtnFrom:(NSInteger)from to:(NSInteger)to {
    ...
    if (_fontScale) {
        ...
        //before_btn.titleLabel.font = ADOBE_FONT(_fontSize);
        //select_btn.titleLabel.font = ADOBE_FONT(_fontSize*_fontScale);
        
        [before_btn setAttributedTitle:[Utils setNavAttributeStringText:before_btn.titleLabel.text AndFontSize:_fontSize AndTextColor:_deSelectColor]
                          forState:UIControlStateNormal];
        [select_btn setAttributedTitle:[Utils setNavAttributeStringText:select_btn.titleLabel.text AndFontSize:_fontSize*_fontScale AndTextColor:_selectColor]
        forState:UIControlStateNormal];
    }
    
    if (lineView) {
        lineView.width = select_btn.width*_lineScale - 10;
        if (self.lineWidth > 0) {
            lineView.width = self.lineWidth;
        }
...
- (void)fontChangeCurBtn:(UIButton *)curBtn nextBtn:(UIButton *)nextBtn changeScale:(CGFloat)changeScale{
    ...
    nextBtn.titleLabel.font = ADOBE_FONT(next_font); //[UIFont systemFontOfSize:next_font];
    curBtn.titleLabel.font = ADOBE_FONT(cur_font); //[UIFont systemFontOfSize:cur_font];
}
