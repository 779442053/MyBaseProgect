...
@implementation CommontsListModel
+ (NSDictionary *)mj_objectClassInArray{
    return @{
             @"data" : [CommontData class],
              @"advertisements" : [Advertisements class]
             };
}
@end

@implementation CommontData

-(CGFloat)cellHeight{
    if (_cellHeight) return _cellHeight;
    CGFloat knowHeight = K_APP_HEIGHT/667*343;
    //label的高度
    CGSize textMaxSize = CGSizeMake([UIScreen mainScreen].bounds.size.width - 20 - 15, MAXFLOAT);
    CGFloat labelHeight = [self.context boundingRectWithSize:textMaxSize options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName : [UIFont systemFontOfSize:14]} context:nil].size.height;
    _cellHeight += labelHeight + knowHeight + 10 + 0;
    return _cellHeight;
...
//        dispatch_queue_t queue = dispatch_queue_create("dispatchGroupMethodd", DISPATCH_QUEUE_CONCURRENT);
