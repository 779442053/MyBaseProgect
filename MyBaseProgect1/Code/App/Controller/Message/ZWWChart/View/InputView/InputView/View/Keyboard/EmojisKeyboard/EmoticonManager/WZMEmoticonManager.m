...
//    [emoticons addObject:array1];
    ...
    NSString *zwpath1 = [[NSBundle mainBundle] pathForResource:@"Emotions" ofType:@"plist"];
    NSDictionary *zwarray1 = [[NSDictionary alloc] initWithContentsOfFile:zwpath1];
    //ZWWLog(@"默认表情数组=====%@",zwarray1)
    //ZWWLog(@"默认表情数组=====%@",array1)
    NSArray *AllkeySARR = zwarray1.allKeys;
    NSMutableArray *arr = [[NSMutableArray alloc]init];
    for (int i = 0; i < AllkeySARR.count ; i++) {
        NSMutableDictionary *parma = [[NSMutableDictionary alloc]init];
        parma[@"chs"] = AllkeySARR[i];
        parma[@"type"] = @"0";
        parma[@"cht"] = AllkeySARR[i];
        parma[@"png"] = [zwarray1 objectForKey:AllkeySARR[i]];
        [arr addObject:parma];
    }
    [emoticons addObject:arr];
...
- (NSMutableAttributedString *)attributedString:(NSString *)aString {
    ...
    if (!error) {
        ...
        for (NSTextCheckingResult *match in matchs) {
            ...
            //NSString *gifimagename = [NSString stringWithFormat:@"%@.gif",imageName];
            UIImage *image = [UIImage imageNamed:imageName];
            //UIImage *image = [UIImage imageWithGIFNamed:imageName];
            if (image) {
