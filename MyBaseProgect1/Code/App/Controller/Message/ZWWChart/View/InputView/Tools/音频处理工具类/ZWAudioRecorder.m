...
@interface ZWAudioRecorder()<AVAudioRecorderDelegate,AVAudioPlayerDelegate>{
...
@end
@implementation ZWAudioRecorder
+ (instancetype)shareInstanced{
    static ZWAudioRecorder *g_instance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        g_instance = [[ZWAudioRecorder alloc] init];
    });
    return g_instance;
}
...
- (void)startRecordingWithFileName:(NSString *)fileName
    ...
    NSError *error = nil;
    if (![self canRecord]) {
        [ZWUtils showAlertWithTitle:@"无法录音" message:@"请在iPhone的“设置-隐私-麦克风”选项中，允许iCom访问你的手机麦克风。" okTitle:@"确定" cancelTitle:nil dismiss:^(BOOL resultYes) {
        }];
        if (completion) {
        ...
        if(errorCategory){
            NSLog(@"%@", [errorCategory description]);
        }
        //2.初始化录音设备
...
- (void)stopRecordingWithCompletion:(void(^)(NSString *recordPath))completion
{
    ...
    if ([_audioRecorder isRecording]) {
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            ...
            NSLog(@"record time duration :%f", recordTimeInterval);
        });
...
{
    if (self.timerUpdateMeters) {
        [self.timerUpdateMeters invalidate];
        self.timerUpdateMeters = nil;
    }
