...
@implementation NetManagner
+ (NetManagner*)sharedInstance {
    static NetManagner  *g_sharedInstance = nil;
    static dispatch_once_t pre = 0;
    dispatch_once(&pre, ^{
        g_sharedInstance = [[NetManagner alloc] init];
    });
...
- (NSString *)requestPathWithUrl:(NSString *)Url params:(NSDictionary *)params protocolName:(NSString *)protocolName{
    ...
    ZWWLog(@"%@ requestPath :%@",protocolName,requestPath);
    return requestPath;
#else
    return nil;
#endif
...
- (BOOL)canParseResponseObject:(NSData *)responseObject jsonObj:(NSDictionary **)jsonObj{
    ...
    if (parseJsonError) {
        ZWWLog(@"json 解析失败:%@",parseJsonError.localizedDescription);
        return NO;
    }
    else {

        if (![jsonObject isKindOfClass:[NSDictionary class]]){
            ZWWLog(@"服务器返回的数据不是字典类型！");
            return NO;
        }
