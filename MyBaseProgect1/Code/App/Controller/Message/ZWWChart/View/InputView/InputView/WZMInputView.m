...
- (CGFloat)startYOfInputView {
    float knowHeight = [[UIDevice currentDevice] isiPhoneX] ?100:130;
    return [UIScreen mainScreen].bounds.size.height-self.inputToolView.bounds.size.height - K_APP_TABBAR_HEIGHT - knowHeight - K_APP_STATUS_BAR_HEIGHT;
}
#pragma mark - 代理事件
//toolView
...
- (WZMEmojisKeyboard *)emojisKeyboard {
    if (_emojisKeyboard == nil) {
        _emojisKeyboard = [[WZMEmojisKeyboard alloc] initWithFrame:CGRectMake(0, _toolView.bounds.size.height - 40, self.bounds.size.width, 200+[WZMInputHelper helper].iPhoneXBottomH)];
...
- (WZMMoreKeyboard *)moreKeyboard {
    if (_moreKeyboard == nil) {
        _moreKeyboard = [[WZMMoreKeyboard alloc] initWithFrame:CGRectMake(0, _toolView.bounds.size.height - 40, self.bounds.size.width, 200+[WZMInputHelper helper].iPhoneXBottomH)];
