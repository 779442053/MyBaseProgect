...
@interface MainListCollectionViewController ()<UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout,WaterfallLayoutDelegate,TYCyclePagerViewDataSource,TYCyclePagerViewDelegate,MoviesDetailsVCDelegate,NLSliderSwitchProtocol>{
...
-(void)viewDidScrollToVisiableArea{
    ZWWLog(@"当前滑动到了‘%@’页面",_strLayouType);
    ...
    if (self.tADData && [self.tADData count] > index) {
        ...
        if ([strInfo hasPrefix:@"http"]) {
            if ([strInfo hasSuffix:@"gif"]) {
                fimg.image = K_APP_DEFAULT_IMAGE_SMALL;
                dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
