...
- (void)addViewCacheIndex:(NSInteger)index {
    ...
    if ([object isKindOfClass:[NSString class]]) {
        ...
        if ([class isSubclassOfClass:[UIViewController class]]) {//vc
            ...
            ZWWLog(@"please enter the correct name of class!");
        }
    } else {
        if ([object isKindOfClass:[UIViewController class]]) {
            ...
            ZWWLog(@"this class was not found!");
        }
    }
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    if ([self.segDelegate respondsToSelector:@selector(scrollEndIndex:)]) {
        ...
        BOOL isLeft = YES;
        BOOL isRight = NO;
        if (start_offset_x >= scrollView.contentOffset.x) {
            isLeft = NO;
            isRight = YES;
        }
        ...
        if(self.scrollEndAction != nil && [self respondsToSelector:@selector(scrollEndAction)]){
            self.scrollEndAction(currentIndex,isLeft,isRight);
        }
    }
