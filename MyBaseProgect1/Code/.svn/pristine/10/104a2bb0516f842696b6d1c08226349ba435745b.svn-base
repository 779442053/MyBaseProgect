...
+ (ZWDownLoadManager *)sharedInstance {
    static ZWDownLoadManager  *g_sharedInstance = nil;
    static dispatch_once_t pre = 0;
    dispatch_once(&pre, ^{
        g_sharedInstance = [[ZWDownLoadManager alloc] init];
    });
...
- (void)_doDownLoadWithRequestUrl:(NSString *)requestUrl model:(YHDownLoadModel *)model maxConcurrentCount:(int)maxConcurrentCount downloadQueue:( NSMutableArray *)downloadQueue saveInDir:(NSString *)saveInDir saveFileName:(NSString *)saveFileName complete:(void (^)(BOOL success,id obj))complete progress:(void(^)(int64_t bytesWritten, int64_t totalBytesWritten))progress{
    WeakSelf
    [[NetManagner sharedInstance] downLoadWithRequestUrl:requestUrl saveInDir:saveInDir saveFileName:saveFileName complete:^(BOOL success, id obj) {
        ...
        if (model.progress) {
            ZWWLog(@"任务：%@,进度：%lld--%lld",[model.filePath lastPathComponent],downloadProgress.completedUnitCount,downloadProgress.totalUnitCount);
