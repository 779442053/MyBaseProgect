...
-(void)setFollowModel:(FollowArr *)FollowModel{
    if (FollowModel) {
        _FollowModel = FollowModel;
        [self.headIMageView sd_setImageWithURL:[NSURL URLWithString:FollowModel.photo] placeholderImage:[UIImage imageNamed:@"我的"]];
        [self.headIMageView wyh_autoSetImageCornerRedius:56/2 ConrnerType:UIRectCornerAllCorners];
        self.MessageNumLB.hidden = YES;
        self.NameLB.text = FollowModel.name;
        self.DesLB.text = [NSString stringWithFormat:@"粉丝数: %ld",(long)FollowModel.fansCount];
        if (FollowModel.followid == 0) {//我没有关注
...
-(void)zw_setupViews{
    ...
    [[self.MessageBtn rac_signalForControlEvents:UIControlEventTouchUpInside] subscribeNext:^(__kindof UIControl * _Nullable x) {
        if (self.FollowModel) {
            [self.Subject sendNext:@{@"code":@"0",@"Model":self.FollowModel}];
        }else if (self.FansModel){
    ...
    [[self.FollowBtn rac_signalForControlEvents:UIControlEventTouchUpInside] subscribeNext:^(__kindof UIControl * _Nullable x) {
        if (self.FollowModel) {
            [self.Subject sendNext:@{@"code":@"1",@"Model":self.FollowModel}];
        }else if (self.FansModel){
