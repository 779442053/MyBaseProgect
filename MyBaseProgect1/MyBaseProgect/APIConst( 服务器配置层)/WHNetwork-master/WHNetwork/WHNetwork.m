...
#pragma mark -
#pragma mark - 上传文件
+ (NSURLSessionTask *)uploadFileWithURL:(NSString *)URL parameters:(id)parameters name:(NSString *)name localFilePath:(NSString *)localFilePath seriverFileName:(NSString *)seriverFileName fileType:(WHUploadFileType)fileType progress:(Progress)progress success:(Success)success failure:(Failure)failure {
    NSURLSessionTask *sessionTask = [_sessionManager POST:URL parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        NSData *fileData = [NSData dataWithContentsOfFile:localFilePath];
        NSString *fileTypeString = @"text/xml";
        switch (fileType) {
            case WHUploadFileTyApeapplicationJSON:
                fileTypeString = @"application/json";
                break;
            case WHUploadFileTypeTextHtml:
                fileTypeString = @"text/html";
                break;
            case WHUploadFileTypeTextJSON:
                fileTypeString = @"text/json";
                break;
            case WHUploadFileTypeTextPlain:
                fileTypeString = @"text/plain";
                break;
            case WHUploadFileTypeTextJavascript:
                fileTypeString = @"text/javascript";
                break;
            case WHUploadFileTypeTextXML:
...
+ (void)setRequestSerializer:(WHRequestSerializer)requestSerializer {
    _sessionManager.requestSerializer = (requestSerializer == WHRequestSerializerHTTP) ? [AFHTTPRequestSerializer serializer] : [AFJSONRequestSerializer serializer];
}

/** 设置响应数据格式，默认JSON */
+ (void)setResponseSerializer:(WHResponseSerializer)responseSerializer {
    _sessionManager.responseSerializer = (responseSerializer == WHResponseSerializerHTTP) ? [AFHTTPResponseSerializer serializer] : [AFJSONResponseSerializer serializer];
}
        ...
        [[AFNetworkReachabilityManager sharedManager] setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
            switch (status) {
                case AFNetworkReachabilityStatusUnknown:
                    networkStatus ? networkStatus(WHNetworkStatusUnknown, @"NetworkStatus Unknow") : nil;
                    break;
                case AFNetworkReachabilityStatusNotReachable:
                    networkStatus ? networkStatus(WHNetworkStatusNotReachable, @"NetworkStatus NotReachable") : nil;
                    break;
                case AFNetworkReachabilityStatusReachableViaWWAN:
                    networkStatus ? networkStatus(WHNetworkStatusWWAN, @"WHNetworkStatus WWAN") : nil;
                    break;
                case AFNetworkReachabilityStatusReachableViaWiFi:
                    networkStatus ? networkStatus(WHNetworkStatusWiFi, @"NetworkStatus WiFi") : nil;
                    break;
            }
        }];
    });
