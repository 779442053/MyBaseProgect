...
//
 ...
 /*
  ...
  [ZWPAYMANAGER ZW_payWithOrderMessage:req callBack:^(ZWErrCode errCode, NSString *errStr) {
  ...
  *  来自支付宝文档数据
  ...
  [ZWPAYMANAGER ZW_payWithOrderMessage:orderMessage callBack:^(ZWErrCode errCode, NSString *errStr) {
  NSLog(@"errCode = %zd,errStr = %@",errCode,errStr);
  }];
  }
  ...
  *
  *  来自微信文档数据==由此可以看出,微信的服务器是PHP写出来的
