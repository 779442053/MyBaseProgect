...
+ (void)showAlertWithButtonClickedBlock:(void (^)())buttonClickedBlock {
    ...
    // 背景图片
    UIImageView *bgImageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"register"]];
...
/** 展示loading图 */
+ (void)showLoading {
    [ZWHUD showLoadingWithMessage:nil];
}
...
+ (void)showLoadingWithMessage:(NSString *)message {
    ...
    [window addSubview:[ZWLoading sharedInstance]];
    [ZWLoading sharedInstance].loadingInfo = message;
    [[ZWLoading sharedInstance] mas_makeConstraints:^(MASConstraintMaker *make) {
        make.edges.mas_equalTo(UIEdgeInsetsMake(0, 0, 0, 0));
    }];
}
...
/** 移除loading图 */
+ (void)dismiss {
    [[ZWLoading sharedInstance] removeFromSuperview];
}
...
+ (void)enableUserInteraction:(BOOL)isEnable {
    [ZWLoading sharedInstance].userInteractionEnabled = !isEnable;
}
...
+ (void)showLoadingWithEnableUserInteraction:(BOOL)isEnable {
    [ZWHUD showLoading];
    [ZWHUD enableUserInteraction:isEnable];
}
...
+ (void)showLoadingWithMessage:(NSString *)message enableUserInteraction:(BOOL)isEnable {
    [ZWHUD showLoadingWithMessage:message];
    [ZWHUD enableUserInteraction:isEnable];
}
